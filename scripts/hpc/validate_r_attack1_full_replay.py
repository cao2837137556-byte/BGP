#!/usr/bin/env python3
"""Validate complete R-ATTACK-1 full-window replay artifacts."""
...
def main() -> None:
    ...
    checks = {
        ...
        "qa_required_attack_subtypes_present": len(qa.get("attack_subtype_counts", {})) >= 2,
    ...
    summary = {
        ...
        "foreground_background_suppression_rate": foreground.get("pure_reference_background_suppression_rate"),
        "foreground_compression_ratio": foreground.get("estimated_compression_ratio"),
        "allowed_claim": "R-ATTACK-1 controlled subprefix and NO_EXPORT/stealth replay is validated if validated=true.",
        "forbidden_claims": [
            "Do not claim NO_EXPORT is attack truth",
            "Do not claim NO_EXPORT absence is safe",
            "Do not claim historical-event recall",
            "Do not claim poisoning/evasion robustness",
            "Do not claim learning performance"
        ],
        "recommended_next_step": (
            "analyze per-family foreground retention and then plan paired poisoning/evasion or historical replay"
            if not failed
            else "repair failed R-ATTACK-1 artifact checks"
        ),
    }
