...
PAIR_ID=${1:-n_frontend1b_$(date -u +%Y%m%dT%H%M%SZ)}
...
RECORD=$BASE/logs/n_frontend1b_pair_${PAIR_ID}.txt

export N_FRONTEND1B_DATE=$REPLAY_DATE
export N_FRONTEND1B_WINDOW_MINUTES=$WINDOW_MINUTES
bash "$REPO/scripts/hpc/n_frontend1b_bounded_replay_preflight.sh" \
  "$PAIR_ID" "$SOURCE_ROOT"
mkdir -p "$BASE/logs"
...
AMD_JOB=$(sbatch --parsable -p amd -J bgp_n_front1b_amd \
...
INTEL_JOB=$(sbatch --parsable -p intel -J bgp_n_front1b_intel \
  --export=ALL,N_FRONTEND1B_PAIR_ID="$PAIR_ID",N_FRONTEND1B_SOURCE_ROOT="$SOURCE_ROOT",N_FRONTEND1B_DATE="$REPLAY_DATE",N_FRONTEND1B_WINDOW_MINUTES="$WINDOW_MINUTES" \
  "$JOB_SCRIPT")
