...
- Current Raw Incident Entry State:
  ...
  - Outputs: `outputs/r_noise_0/s2a_baseline_v01_pilot_6h_april16/r_noise_0_summary.json`, noise feature availability, multi-attack must-keep coverage, counterfactual suppression policy audit, clean-window noise profile, legacy workflow reference audit, poisoning/evasion retention audit, and sample audit CSVs.
  - R-NOISE-0 is an audit only: no row deletion, no background suppression implementation, no old pipeline modification, no learning training, and no truth label.
  - R-NOISE-0 result: candidate-entry rows `3431103`; must_keep `1812334` (`0.528207`); `policy_A_very_conservative` would suppress `1614840` rows (`0.470647`) and keep `1816263` rows; estimated compression ratio `1.889100`.
  ...
  - R-AGG-3 is an audit only: it does not modify R-AGG-2 aggregation logic, does not implement safe merge, does not implement background suppression, does not attach external evidence, and does not train learning.
  ...
  - R-AGG-2 uses candidate-entry as the primary source and event-entry time repair through `event_id` exact join. It does not use final-entry as the main source.
  - Full result: candidate rows `3431103`, event rows `3431103`, prototype raw incidents `3128971`, compression ratio `1.09656`, time_scope coverage `1.0`, exact_join `3128971`, evidence_grounding_ready_rate `0.948727`.
  - family_hint distribution: `mixed_unknown=1707486`, `stealth_visibility_like=1406742`, `forged_origin_like=14743`. family_hint is semantic hint, not attack label.
  - visibility is dominated by `single_collector=3089300`; this is observation scope, not stealth truth.
  - Current risk: key v0 is conservative and under-aggregates; R-AGG-3 has confirmed safe merge space exists, but family_hint repair must come first.
  - Next recommended step is R-AGG-4 family_hint mapping repair before safe merge / pre-incident filter design, with Evidence-grounded Incident design as the next branch after aggregation quality is under control.
  ...
  - R-AGG-2 built the first prototype table. R-AGG-3 has audited its aggregation quality and recommends family_hint mapping repair before coarser grouping.
